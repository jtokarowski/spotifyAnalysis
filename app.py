...
def authed():
    ...
    for track in DJSET:
        for audioFeature in spotifyAudioFeatures:
            ...
            try:
                trackTargets[key] = track[audioFeature]
                
                minKey = "min_{}".format(audioFeature)
                maxKey = "max_{}".format(audioFeature)
                trackTargets[minKey] = track[audioFeature] - bound
                trackTargets[maxKey] = track[audioFeature] + bound

                newSetTargets.append(trackTargets)
            except:
                trackTargets[key] = 0.5
    ...
    for artist in userTopArtists:  
        ...
        for track in recommendedTracks:
            if track['trackID'] not in masterTrackPoolIDs:
                masterTrackPool.append(spotifyDataRetrieval.getAudioFeatures(track))
                masterTrackPoolIDs.append(track['trackID'])

    print("Loaded {} track recommendations".format(len(masterTrackPoolIDs)))
    ...
    for targetTrack in newSetTargets:
        ...
        for recommendedTrack in masterTrackPool:
            ...
            for audiofeature in spotifyAudioFeatures:
                ...
                if audiofeature not in skipFeatures:
                    diff = (recommendedTrack['audioFeatures'][feature]*100) - (targetTrack["target_{}".format(audioFeature)]*100)
                    totaleEuclideanDistance += diff * diff
                    rec["TED"] = TED
            #append the distance to a list that aligns with the pool
            totalEuclideanDistances.append(totaleEuclideanDistance)

        # sort the recommendations by min ED
        bestFitTrack = masterTrackPool[totalEuclideanDistances.index(min(totalEuclideanDistances))]
